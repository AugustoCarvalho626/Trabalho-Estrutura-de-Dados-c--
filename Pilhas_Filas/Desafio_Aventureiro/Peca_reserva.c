// ======================================================================================
// NOME DO PROJETO: Tetris Stack - Incluido uma pilha de peça reserva
...
//                  a entrada e saída de peças e usando sistema de pilha para reservar peças.
...
/**
 * Estrutura de controle da Pilha de Reservas.
 */
typedef struct {
    Peca itens[TAM_FILA]; // Reutiliza o tamanho da fila para a pilha
...
int main() {
    ...
    inicializarPilha(&pilhaDeReservas);
    ...
    do {
        ...
        switch (opcao) {
            ...
            case 2:
                ...
                break;
            case 3:
                // Reservar peça (push): Tira da fila e põe na pilha
                if (filaDePecas.total > 0 && pilhaDeReservas.topo < TAM_FILA - 1) {
                    Peca p = filaDePecas.itens[filaDePecas.inicio];
                    filaDePecas.inicio = (filaDePecas.inicio + 1) % TAM_FILA;
                    filaDePecas.total--;
                    push(&pilhaDeReservas, p);
                } else if (filaDePecas.total == 0) {
                    printf("\nAVISO: Fila vazia! Nada para reservar.\n");
                } else {
                    printf("\nAVISO: Pilha cheia! Use uma peça reservada antes.\n");
                }
                break;
            case 4:
...
void push(Pilha *p, Peca peca) {
    // A verificação de pilha cheia é feita no main antes de chamar, 
    // mas por segurança mantemos o controle do índice.
    if (p->topo < TAM_FILA - 1) {
        p->topo++;
        p->itens[p->topo] = peca;
        printf("\n--> Peça [%c %d] movida para a reserva (Push)!\n", peca.nome, peca.id);
    }
}

void pop(Pilha *p) {
    if (p->topo == -1) {
        printf("\nAVISO: A pilha de reserva está vazia!\n");
        return;
    }
    Peca peca = p->itens[p->topo];
    p->topo--;
    printf("\n<-- Peça de reserva [%c %d] utilizada (Pop)!\n", peca.nome, peca.id);
}
